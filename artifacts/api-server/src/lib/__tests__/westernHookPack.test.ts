...
describe("W2-A — corpus shape (dark)", () => {
  ...
  it("draft hook layer (dupes, weak skeletons, scenario, privacy) is clean", () => {
    ...
    expect(
      r.failures.filter((f) => f.code === "setting_invalid"),
      "no setting failures",
    ).toEqual([]);
    expect(
    ...
    // After the W2-Batch-C reviewer-approved comedyFamily swap on
    // W2C-028 and W2C-040, the full 150-entry draft corpus must
    // pass the integrity check with zero surfaced failures.
