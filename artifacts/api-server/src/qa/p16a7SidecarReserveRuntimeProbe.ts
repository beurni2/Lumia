...
async function upsertCreator(
  ...
  const name = `qa_sweep:p16a7_sidecar_reserve:${PASS}:${cohort}:${idx}`;
...
function makeStubCreator(
  ...
  return {
    id,
    authUserId: null,
    name: `qa_sweep:p16a7_sidecar_reserve:${PASS}:${cohort}:${idx}`,
...
const OUT_DIR = path.join(REPO_ROOT, ".local", "qa-runs", "p16a7");
...
async function runCohortChunk(cohort: Cohort): Promise<void> {
  ...
  for (let i = cStart; i < cEnd; i++) {
    ...
    const id = uuidFromString(`p16a7:${PASS}:${cohort}:c${i}`);
