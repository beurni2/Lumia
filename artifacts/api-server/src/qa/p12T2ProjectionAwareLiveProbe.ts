/**
 * P12 — T2 PROJECTION-AWARE-ATTEMPT-BUDGET LIVE PROBE.
 *
 ...
 *   pnpm exec tsx .local/scripts/p12T2LiveRouteReAudit.mts
 */
...
const API_URL = process.env.P12_LIVE_API_URL ?? "http://localhost:8080/api/ideator/generate";
...
async function resetDemoCreatorMemory(): Promise<void> {
  ...
  if (!demo) throw new Error("[p12] no demo creator row found — cannot reset memory");
  await db
...
//
// Env contract:
//   P12_COHORT=ng_light_pidgin|ng_clean|western|all
//     Runs that single cohort and writes its partial JSON; default `all`.
//   P12_MODE=run|aggregate
//     `run` (default): runs the selected cohort(s), persists
//       `_p12_partial_<cohort>.json`. `aggregate`: reads partials of
...
async function runCohort(c: Cohort): Promise<{ ideas: IdeaRec[]; batches: BatchRec[] }> {
  ...
  // of overwriting. P12_CREATOR_OFFSET / P12_CREATOR_LIMIT trim which
  // creators this invocation runs.
  const partialPath = path.join(QA_RUNS_DIR, `_p12_partial_${c.label}.json`);
  let ideas: IdeaRec[] = [];
  ...
  if (fs.existsSync(partialPath)) {
    try {
      ...
      console.log(`[p12] resumed cohort=${c.label} priorIdeas=${ideas.length} priorBatches=${batches.length}`);
    } catch { /* start fresh */ }
  }
  const offset = Number(process.env.P12_CREATOR_OFFSET ?? "0");
  const limit = Number(process.env.P12_CREATOR_LIMIT ?? String(c.creators));
  ...
  if (ideas.length !== beforeIdeas || batches.length !== beforeBatches) {
    console.log(`[p12] dropped prior creator-slice records to keep resume idempotent: ideas ${beforeIdeas}→${ideas.length}, batches ${beforeBatches}→${batches.length}`);
  }
  for (let creatorIdx = start; creatorIdx < stop; creatorIdx++) {
    ...
    if (resetErr) {
      console.log(`[p12] cohort=${c.label} cre=${creatorIdx + 1} memory-reset-failed-after-3-attempts err=${String((resetErr as Error).message ?? resetErr).slice(0, 120)} — continuing without reset`);
    }
    ...
    for (let batchIdx = 0; batchIdx < c.batchesPerCreator; batchIdx++) {
      ...
      console.log(`[p12] cohort=${c.label} cre=${creatorIdx + 1}/${c.creators} batch=${batchIdx + 1}/${c.batchesPerCreator} ms=${r.durationMs} status=${r.status} ideas=${ideaCount} fb=${r.resp?.usedFallback ?? "?"} fbReason=${r.resp?.qaTelemetry?.fallbackDecision?.reason ?? "?"}${r.err ? ` err=${r.err.slice(0, 80)}` : ""}`);
      // Persist progress after every batch — even if the next batch
      // crashes the process we still have audit data on disk.
      try {
        fs.writeFileSync(path.join(QA_RUNS_DIR, `_p12_partial_${c.label}.json`), JSON.stringify({ ideas, batches }, null, 2));
      } catch { /* ignore */ }
    }
  }
...
async function runMode(cohortFilter: string): Promise<void> {
  console.log(`[p12] mode=run cohort=${cohortFilter}`);
  console.log(`[p12] API_URL=${API_URL}`);
  console.log(`[p12] flags: NG_PACK=${process.env.LUMINA_NG_PACK_ENABLED} T2=${process.env.LUMINA_NG_PACK_PROJECTION_T2_ENABLED} PAR=${process.env.LUMINA_NG_PACK_AWARE_RETENTION_ENABLED} SOFT=${process.env.LUMINA_NG_MEMORY_SOFT_CAP_ENABLED}`);
  console.log(`[p12] NIGERIAN_HOOK_PACK.length=${NIGERIAN_HOOK_PACK.length}, FOOD_V2 ids=${FOOD_V2_IDS.size}`);
  ...
  for (const c of selected) {
    console.log(`[p12] === cohort start: ${c.label} (creators=${c.creators}, batches/cre=${c.batchesPerCreator}, ideas/batch=${c.ideasPerBatch}) ===`);
    const r = await runCohort(c);
    fs.writeFileSync(path.join(QA_RUNS_DIR, `_p12_partial_${c.label}.json`), JSON.stringify(r, null, 2));
    console.log(`[p12] === cohort done: ${c.label} ideas=${r.ideas.length} batches=${r.batches.length} ===`);
  }
}

function loadPartial(label: string): { ideas: IdeaRec[]; batches: BatchRec[] } {
  const p = path.join(QA_RUNS_DIR, `_p12_partial_${label}.json`);
  if (!fs.existsSync(p)) {
    console.warn(`[p12] missing partial for cohort=${label} — using empty`);
    return { ideas: [], batches: [] };
  }
...
async function aggregateMode(): Promise<void> {
  console.log("[p12] mode=aggregate — loading partials + writing final outputs");
  const results = new Map<string, { ideas: IdeaRec[]; batches: BatchRec[] }>();
  // Strict aggregator (architect feedback): every COHORTS entry must
  // have a partial on disk unless P12_ALLOW_PARTIAL_AGGREGATE=1 is set
  // explicitly. Prevents silent "looks complete" final outputs.
  const missing: string[] = [];
  for (const c of COHORTS) {
    const p = path.join(QA_RUNS_DIR, `_p12_partial_${c.label}.json`);
    if (!fs.existsSync(p)) missing.push(c.label);
    results.set(c.label, loadPartial(c.label));
  }
  if (missing.length > 0 && process.env.P12_ALLOW_PARTIAL_AGGREGATE !== "1") {
    console.error(`[p12] FATAL: missing partials for cohort(s) ${missing.join(", ")}. Set P12_ALLOW_PARTIAL_AGGREGATE=1 to override.`);
    process.exit(3);
  }
  ...
  fs.writeFileSync(path.join(QA_RUNS_DIR, "p12_t2_live_route_re_audit.json"), JSON.stringify(audit, null, 2));
  fs.writeFileSync(path.join(QA_RUNS_DIR, "p12_t2_live_route_re_audit_summary.json"), JSON.stringify({ meta: audit.meta, aggregate }, null, 2));
  console.log(`[p12] wrote ${QA_RUNS_DIR}/p12_t2_live_route_re_audit{,_summary}.json (totalRunMs=${totalRunMs})`);
  console.log(`[p12] ng_lp: total=${aggregate.ng_light_pidgin.totalIdeas} pack=${aggregate.ng_light_pidgin.packIdeas} (${(aggregate.ng_light_pidgin.packShare * 100).toFixed(1)}%) distinctHooks=${aggregate.ng_light_pidgin.distinctHooks}`);
  console.log(`[p12] home→food shipped=${aggregate.widenedEdges["home→food"].shipped} distinctPackIds=${aggregate.widenedEdges["home→food"].distinctPackIds}`);
  console.log(`[p12] FOOD_V2 shippedAtLeastOnce=${aggregate.foodV2.shippedAtLeastOnce}/${aggregate.foodV2.importedTotal} totalCards=${aggregate.foodV2.totalShippedCards}`);
}

async function main(): Promise<void> {
  const mode = process.env.P12_MODE ?? "run";
  if (mode === "aggregate") { await aggregateMode(); return; }
  if (mode !== "run") throw new Error(`unknown P12_MODE: ${mode}`);
  const cohort = process.env.P12_COHORT ?? "all";
  // Pre-flight gated by P12_PREFLIGHT=1 so chunked invocations after
  ...
  if (process.env.P12_PREFLIGHT === "1") {
    ...
    console.log(`[p12] pre-flight telemetry visible: ${telemetryOk} (status=${probe.status} ms=${probe.durationMs})`);
    if (!telemetryOk) { console.error("[p12] FATAL: qaTelemetry not visible"); process.exit(2); }
  }
