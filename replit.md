...
- **N1 Trigger Synthesis (PHASE N1-TRIGGER-FIX 2026-05-06):** `nigerianPackAuthor.ts` `extractShowContentTokens()` borrows up to 2 unique non-stopword non-anchor content tokens from `entry.whatToShow` and weaves them into the trigger sentence (`notice the {anchor} land while {tok1} {tok2} settle`). Guarantees `triggerOverlap ≥ anchor + 2` for the catalog `hook_scenario_mismatch` validator (comedyValidation.ts L568, ≥2 token overlap rule UNCHANGED). Tokenization regex + `STOPWORDS` set imported from comedyValidation.ts to keep both sides byte-aligned. Empirical impact: pack-validator drop 53.2%→8.3%, hook_scenario_mismatch 3315→0 rejections; staging QA combined fill 31→**39/60 median** across all 5 seeds (range 39-42, all GO, all correctness gates clean). Pure additive overlay inside the pack author; cohort-gated upstream so Western/India/PH/NG-clean cohorts unaffected.
