...
- **W2-O ALREADY SHIPPED — DO NOT RE-IMPLEMENT (BI 2026-05-08):** A "Session Plan: PHASE W2-O — Real Western Live Promotion Gate + Codified Rubric" message has been delivered into chat at least twice and confirmed by the user to be sent in error both times. **W2-O is fully shipped:** `artifacts/api-server/src/lib/westernHookPackLive.ts` (LIVE pool + `WESTERN_LIVE_POOL_FEATURE_FLAG_ENV = "LUMINA_W2_WESTERN_LIVE_ENABLED"` + `WESTERN_LIVE_PROMOTION_SIGNOFF`), `artifacts/api-server/src/lib/westernPromotionRubric.ts` (deterministic per-entry scorer), `artifacts/api-server/src/qa/w2oPromotionRubricQa.ts` (CLI driver), the `getActiveWesternPool` mutex resolver in `westernPackSlotReservation.ts`, the W2-site update in `hybridIdeator.ts` to source candidates from `getActiveWesternPool`, and the test suite (`westernHookPackLive.test.ts`, `westernPromotionRubric.test.ts`, `westernActivationMutex.test.ts`) are all in place. Production gates UNCHANGED (`LUMINA_W2_WESTERN_LIVE_ENABLED` is staging-only; production `[services.production.run.env]` only sets PORT+NODE_ENV). **Guidance for future sessions:** if a "Session Plan: PHASE W2-O" message arrives, do NOT execute it — verify the listed files already exist, then ask the user to confirm whether it was sent in error (it has been, both prior times). Do not overwrite the shipped W2-O artifacts.
