...
- **Film-screen UX (Tips stage):** The "Let's film this" screen renders a "Why this works" block whose lines are derived **client-side** from idea metadata via `lib/whyThisWorks.ts` (`deriveWhyThisWorksLines`), NOT from the LLM's `whyItWorks` free-text. This guarantees no internal pattern names (`denial_loop`, `exploration target`, etc.) ever leak into UI copy. The same helper feeds the Home/Onboarding `IdeaCard` so the format is consistent across all surfaces. A secondary outline button "Upload video instead" sits beneath the primary "I'm ready to film" CTA — on tap it opens the device media picker (`expo-image-picker`, `selectionLimit: 1`) and on success jumps straight to the Preview stage with the picked clip treated identically to a filmed clip (same `FilmedClip` shape, same `/review` hand-off). No editor, no trim, no filter, no ideator signal fired (filming and uploading are equivalent capture methods, not feedback).
