...
- **Phase 1 multi-clip rule (do not violate):** The import step of `app/create.tsx` accepts *one or more* clips per idea — the OS gallery picker is configured with `allowsMultipleSelection: true` (capped at 8) so a creator can pull in a problem-clip + solution-clip in a single round trip. However, **templates still render a single short-form video output**: `clips[0]` is the canonical primary, the remainder are carried through to `/review` via the new `clips` route param as data only. There is no reorder / trim / transition / sequencing UI in Phase 1 — that's Phase 2 ("multi-clip structuring"). The `/review` screen prefers `params.clips` and falls back to legacy `params.clip` for back-compat with deep links. When Phase 2 lands, the data shape already exists; UI is the only addition. The web QA picker mock returns a single-element array so the browser test path stays identical to single-clip.
