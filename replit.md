...
- **N1 Throttle Instrumentation (PHASE N1-INSTRUMENT v2):** `artifacts/api-server/src/qa/instrumentNigerianThrottle.ts` (additive observer in `coreCandidateGenerator.ts` L959-1055, gated by `globalThis.__nigerianThrottleObserver`, no-op when unset). Single sweep produces three sections: throttle-gate measurements (Part 1), per-validator drop accounting (Part 1.5 — counts each `r.reason` and surfaces sample rejected hooks), non-pack style audit (Part 2). Pattern list imported from `nigerianStylePenalty.ts` (single source of truth).
- **N1 Trigger Synthesis (PHASE N1-TRIGGER-FIX + N1-TRIGGER-FIX-B 2026-05-06):** `nigerianPackAuthor.ts` `extractShowContentTokens()` borrows up to 2 unique non-stopword non-anchor content tokens from `entry.whatToShow` and weaves them into the trigger sentence (`notice the {anchor} land while {tok1} {tok2} settle`). Guarantees `triggerOverlap ≥ anchor + 2` for the catalog `hook_scenario_mismatch` validator (comedyValidation.ts L568, ≥2 token overlap rule UNCHANGED). Tokenization regex + `STOPWORDS` set imported from comedyValidation.ts to keep both sides byte-aligned. **N1-TRIGGER-FIX-B: two-pass picker** — Pass 1 prefers "noun-ish" tokens via `TRIGGER_TOKEN_SKIP` (curated set of pronouns, scene-direction imperative verbs, locative connectives, apostrophe forms — broader than STOPWORDS, used ONLY by the picker for readability, never the validator), Pass 2 falls back to the original any-2-content-tokens behavior so we never regress past pre-B picks. Empirical impact: pack-validator drop 53.2%→8.3%, hook_scenario_mismatch 3315→0 rejections; staging QA combined fill 31→**39/60 median** across all 5 seeds (range 39-42, all GO, all correctness gates clean). Pre-B/post-B fill rates identical; B improves trigger readability (~4/14 borderline samples cleaned: "ask who's"→"free weekend", "open fake"→"fake chat", "lands smile"→"list bills", "watch own"→"fake awkward"). Pure additive overlay inside the pack author; cohort-gated upstream so Western/India/PH/NG-clean cohorts unaffected.
