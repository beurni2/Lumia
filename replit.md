...
- **Catalog Skeleton Swap — Relaxed Fallback (BI 2026-05-07):** In `hybridIdeator.ts` (post-pack-reservation skeleton dedup, ~L4789), the swap now has a two-tier alt search. Primary: any non-pack `merged` candidate whose skeleton is novel (unseen) AND not already used in-batch. Fallback: any non-pack `merged` candidate whose skeleton differs from the repeating one AND isn't already used in-batch (even if also seen). Rationale: when the per-creator seen-set saturates the local pool's skeleton space (after ~3 batches), strict swap finds no truly novel alt and the repeating hook ships again — the root cause of the observed 4× repeat of "my body quit. my brain kept screaming" across 10 batches. Rotating among "seen" alts is strictly better than re-shipping the same skeleton. SWAP-ONLY semantics preserved (never drops, never shrinks pool); validator/scorer/anti-copy untouched. Result on demo creator: max catalog repeat 4× → 2×, pack delivery 75% → 78%, refresh ≥95.8%, 0 leaks.
