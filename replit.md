...
- **N1 Native Reviewer Worksheet (read-only export, Top 50 only):** `artifacts/api-server/src/qa/nigerianPackReviewerWorksheet.ts` → `.local/REGIONAL_N1_REVIEWER_WORKSHEET.{md,csv}`. Selects 50 safest activation candidates (light_pidgin/pidgin · anchor in hook AND whatToShow · no mocking · field bounds OK) and emits a worksheet with reviewer-editable columns: `DECISION` (approve/rewrite/reject), `FINAL_PIDGIN_LEVEL`, `REVIEWED_BY`, `NOTES`, `REWRITTEN_*`.
- **N1 Rewrite Worksheet (read-only export, rejected rows):** `artifacts/api-server/src/qa/nigerianPackRewriteWorksheet.ts` → `.local/REGIONAL_N1_REWRITE_WORKSHEET.{md,csv}`. Re-runs the same per-row validation as the approved-pack builder and emits ONLY the rejected rows with full original context, `scoreHookQuality` value, failure reason, and blank `rewrittenHook` (REQUIRED) + optional `rewrittenWhatToShow`/`rewrittenHowToFilm`/`rewrittenCaption`/`rewriteNotes` columns. Sorted by hookScore desc (closest to passing first). Does NOT mutate drafts, approved candidates, or the live pack; does NOT lower `HOOK_QUALITY_FLOOR`. Applies the same `.local/REGIONAL_N1_REWRITES.yaml` overlay as the approved-pack builder, so re-running after a rewrite pass shows the *current* hook + score for each still-failing row (rows already cleared by rewrites are absent).
- **N1 Approved Promotion Candidates (INERT, NOT in live pack):** `artifacts/api-server/src/lib/nigerianHookPackApproved.ts` (auto-generated by `qa/buildApprovedNigerianPack.ts`). Exports `APPROVED_NIGERIAN_PROMOTION_CANDIDATES: readonly NigerianPackEntry[]` — only entries that re-pass EVERY production rule: reviewedBy non-empty + non-sentinel, pidginLevel ∈ {light_pidgin, pidgin}, length bounds, anchor-in-hook AND anchor-in-whatToShow, no mocking patterns, `validateScenarioCoherence === null`, `scoreHookQuality >= 40`. Module-load `assertNigerianPackIntegrity` re-runs the boot rules. **Status: INERT** — array is NOT `NIGERIAN_HOOK_PACK`, the live pool is still `Object.freeze([])`, no recipe wiring, env flag still default-off. First ingest pass (reviewer stamp `BI 2026-05-05`, decision = approve all 50): **4 approved / 46 rejected** — the rejection wave is dominated by `scoreHookQuality < 40`. The rejected list is preserved in the file's leading doc-comment as the audit trail; the generator does NOT silently fix anything. Round 2 (rewrites overlay from `.local/REGIONAL_N1_REWRITES.yaml`): **13 approved / 37 rejected** — 9 reviewer rewrites cleared the floor. Round 3 (Pidgin-leaning rewrites for the 37 still-failing rows, appended to the YAML — last-write-wins by `draftId`): **21 approved / 29 rejected** — 8 more rewrites cleared the floor; 29 still below `scoreHookQuality < 40` floor. All round-3 `validateScenarioCoherence` failures from round 2 (DRAFT-011/171/202) were resolved. Approved entries flagged with `[REWRITE]` in source comments + audit trail.
