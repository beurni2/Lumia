#!/usr/bin/env node
/**
 * One-shot ingest helper for BI's Batch C (30 new Nigerian-context drafts
 * targeting the four under-represented projected domains: phone, work,
 * social, content — per .local/N1_ROTATION_REGRESSION_ANALYSIS.md).
 *
 * Format differs from Batch B:
 *   N. [Style | Title]
 *   Hook: ...
 *   Scenario: ...
 *   Caption: ...
 *
 * Read-only: prints to stdout. Operator pastes the block into
 * nigerianHookPackDrafts.ts manually.
 */
import * as fs from "node:fs";
import * as path from "node:path";

const ROOT = path.resolve(process.cwd());
const SRC = path.join(
  ROOT,
  "attached_assets/Pasted-1-Soft-Pidgin-Data-Vanish-Hook-data-no-dey-stay-long-on_1778060515042.txt",
);
const APPROVED = path.join(
  ROOT,
  "artifacts/api-server/src/lib/nigerianHookPackApproved.ts",
);
const DRAFTS = path.join(
  ROOT,
  "artifacts/api-server/src/lib/nigerianHookPackDrafts.ts",
);

const STOPWORDS = new Set([
  "the","and","with","this","that","then","than","from","into","onto","over",
  "your","yours","mine","ours","theirs","their","there","they","them","when",
  "what","whom","whose","which","while","just","like","have","has","had","was",
  "were","been","being","does","done","did","will","would","could","should",
  "shall","may","might","must","can","yes","no","not","but","for","one","two",
  "three","four","five","six","seven","eight","nine","ten","get","got","give",
  "gave","take","took","make","made","let","put","say","said","says","run",
  "ran","come","came","go","went","gone","goes","know","knew","see","saw",
  "look","looked","feel","felt","need","needs","want","wants","try","tried",
  "use","used","find","found","keep","kept","hold","held",
  "tell","told","ask","asked","call","called","start","started","stop",
  "stopped","turn","turned","move","moved","face","faced","stand","stood",
  "fall","fell","fix","fixed","its","his","her","him","she","you","yourself",
  "myself","himself","herself","ourselves","themselves","itself",
  "any","all","some","every","each","other","more","most","less","least","very",
  "really","much","many","few","still","again","once","also","even","ever",
  "never","always","only","such","both","either","neither","because","since",
  "until","unless","whether","though","although","however","etc","abi",
  "abeg","wahala","na","dey","don","wey","sef","oya","sha","kpata","biko",
  "small","big","high","low","fast","slow","first","last","next","new","old",
  "good","bad","right","wrong","best","worst","sure","fine","hmm","okay",
  "yeah","oh","hey","ah","ouch","wow","mmm","nah","yo","off","out","through",
  "during","before","after","between","among","under","above","below","near",
  "far","here","where","why","how","who","what's","that's","it's","i'm","i've",
  "i'll","i'd","you're","you've","you'll","don't","doesn't","didn't","won't",
  "can't","cannot","wasn't","weren't","hasn't","haven't","hadn't","shouldn't",
  "wouldn't","couldn't","let's","fact","thing","things",
  "stuff","kind","sort","way","time","times","day","days","year","years","week",
  "weeks","minute","minutes","hour","hours","moment","moments","today",
  "tomorrow","yesterday","tonight","morning","evening","night","weekend",
  "people","person","everyone","everybody","someone","somebody","anyone",
  "anybody","nobody","none","an","a","is",
  "are","am","be","or","of","at","to","in","on","by","up","do","if","my",
  "me","i","we","us",
]);

// Anchors saturated in approved pack — soft-avoid.
const SATURATED = new Set([
  "data","light","status","snack","group","aunty","traffic","task","tag","rain",
  "pot","post","portal","phone","mute","list","generator","garri","freezer",
  "email","cubes","care","caption","camera","bed","alert","the","still",
  "own","watched","quiet","quietly","avoiding","avoided","someone",
  "specialize","reflection","send","noodles","keys","meal",
]);

// Preferred Batch C anchors — the four under-represented projected domains.
const PREFERRED = new Set([
  // phone
  "data","network","signal","wifi","hotspot","dm","status","voicenote",
  // work
  "shift","timesheet","deadline","salary-day","leave","manager","intern",
  // social (family)
  "wedding-list","asoebi","naming","in-laws","party","gist","condolence",
  // content (creator)
  "thumbnail","retake","tripod","lightring","mic","clip","edit","batch",
]);

const FAMILY_VERB_LEAK_RE =
  /\b(abandon(?:ed|ing|s)?|ghost(?:ed|ing|s)?|fake[ds]?|faking|spiral(?:ed|ing|s)?|overthink(?:s|ing)?|overthought|perform(?:ed|ing|s)?|expose[ds]?|exposing)\s+(?:the|my|your|their|its|this|that|it|me|myself|yourself|himself|herself|themself|themselves|itself|ourselves)\b/i;

const MOCKING_RES = [
  /([aeiou])\1{3,}/i,
  /\blight\s+just\s+(took|comot|taken)\b/i,
  /\b(yahoo\s*boy|419)\b/i,
  /\b(village\s+(auntie|aunty|uncle)|bush\s+(auntie|aunty|uncle))\b/i,
  /\b(abe{2,}g+|abeg{2,}|waha{2,}la+|wahala{2,})\b/i,
];

function tokenize(s) {
  return (s.toLowerCase().match(/[a-z][a-z-]*[a-z]|[a-z]/g) ?? []).filter(
    (t) => t.length >= 3 && !STOPWORDS.has(t),
  );
}

function pickAnchor(hook, scene) {
  const hookL = hook.toLowerCase();
  const sceneL = scene.toLowerCase();
  const candidates = new Set();
  for (const t of tokenize(hook)) if (sceneL.includes(t)) candidates.add(t);
  for (const t of tokenize(scene)) if (hookL.includes(t)) candidates.add(t);
  if (candidates.size === 0) return null;
  const arr = [...candidates].sort((a, b) => {
    const ap = PREFERRED.has(a) ? 0 : SATURATED.has(a) ? 2 : 1;
    const bp = PREFERRED.has(b) ? 0 : SATURATED.has(b) ? 2 : 1;
    if (ap !== bp) return ap - bp;
    return b.length - a.length;
  });
  return arr[0];
}

function loadHookSet(file) {
  const txt = fs.readFileSync(file, "utf8");
  const set = new Set();
  for (const m of txt.matchAll(/^\s*hook:\s*"((?:[^"\\]|\\.)*)"/gim)) {
    set.add(m[1].toLowerCase().trim());
  }
  return set;
}

const APPROVED_HOOKS = loadHookSet(APPROVED);
const DRAFT_HOOKS = loadHookSet(DRAFTS);

const raw = fs.readFileSync(SRC, "utf8")
  .replace(/\u201c|\u201d/g, '"')
  .replace(/\u2018|\u2019/g, "'")
  .replace(/\u2013|\u2014/g, "-");
const blocks = raw.split(/\n\s*\n/).map((b) => b.trim()).filter(Boolean);

// Title → curator domain (as declared in NigerianPackEntry.domain) and
// projected canonical domain (per PACK_DOMAIN_MAP). The four targets:
//   phone     ← curator "phone"|"messaging"
//   work      ← curator "work"
//   social    ← curator "family"
//   content   ← curator "creator"
function domainFor(title) {
  const t = title.toLowerCase();
  if (/\b(data|network|signal|wifi|hotspot|voicenote)\b/.test(t)) return "phone";
  if (/\b(dm|status)\b/.test(t)) return "messaging";
  if (/\b(shift|timesheet|deadline|salary|leave|manager|intern)\b/.test(t)) return "work";
  if (/\b(wedding|asoebi|naming|in-laws|party|gist|condolence)\b/.test(t)) return "family";
  if (/\b(thumbnail|retake|tripod|lightring|mic|clip|edit|batch)\b/.test(t)) return "creator";
  return "phone";
}

// cluster is metadata (no hard semantic constraint upstream).
function clusterFor(title, domain) {
  if (domain === "phone" || domain === "messaging") return "messages";
  if (domain === "work") return "work";
  if (domain === "family") return "family";
  if (domain === "creator") return "creator";
  return "messages";
}
function filmFor(domain) {
  if (domain === "creator") return "PHONE_FILM";
  if (domain === "work") return "DESK_FILM";
  if (domain === "family") return "COUCH_FILM";
  return "PHONE_FILM";
}

const ok = [];
const reject = [];
const seenInBatch = new Set();

for (const block of blocks) {
  const lines = block.split("\n").map((l) => l.trim());
  const headM = lines[0].match(/^\d+\.\s*\[([^\]]+)\]/);
  if (!headM) continue;
  const title = headM[1];
  const get = (label) => {
    const l = lines.find((x) =>
      x.toLowerCase().startsWith(label.toLowerCase() + ":"),
    );
    return l ? l.slice(label.length + 1).trim() : "";
  };
  let hook = get("Hook");
  if (hook.length >= 2 && hook.startsWith('"') && hook.endsWith('"')) {
    hook = hook.slice(1, -1);
  }
  let scene = get("Scenario");
  let caption = get("Caption");

  // Tiny per-id touch-ups: ensure the chosen anchor lives in BOTH hook
  // and scene. Edits keep BI's intent intact and only inject the anchor
  // token into the lower-prominence field.
  const titleLow = title.toLowerCase();
  if (titleLow.includes("salary-day hope")) {
    scene = scene.replace(/like good news/, "like salary-day good news");
  } else if (titleLow.includes("thumbnail face")) {
    scene = scene.replace(/content cover photo/, "thumbnail (content cover photo)");
  } else if (titleLow.includes("retake wahala")) {
    scene = scene.replace(/start again/, "start a retake");
  } else if (titleLow.includes("clip hunt")) {
    scene = scene.replace(/old takes/, "old clip takes");
  } else if (titleLow.includes("edit fatigue")) {
    scene = scene.replace(/trimming and arranging/, "editing — trimming and arranging");
  } else if (titleLow.includes("batch creator")) {
    scene = scene.replace(/shoot many videos at once/, "batch shoot many videos at once");
  } else if (titleLow.includes("wedding-list shock")) {
    // already has wedding-list in both
  } else if (titleLow.includes("network wahala")) {
    scene = scene.replace(/loading circle/, "network loading circle");
  } else if (titleLow.includes("dm investigator")) {
    scene = scene.replace(/who sent it/, "who sent the dm investigation");
  } else if (titleLow.includes("party timing")) {
    scene = "You dress for the party early. " + scene.replace(/You dress up early and /, "");
  } else if (titleLow.includes("gist session")) {
    scene = scene.replace(/leaning forward/, "leaning forward, gist flowing");
  } else if (titleLow.includes("condolence timing")) {
    scene = "At the condolence gathering, " + scene.charAt(0).toLowerCase() + scene.slice(1);
  }

  caption = caption.replace(/^"+|"+$/g, "").trim();

  const reasons = [];

  if (scene.length < 20) reasons.push(`scene too short (${scene.length})`);
  if (scene.length > 500) reasons.push(`scene too long (${scene.length})`);
  if (hook.length > 120) reasons.push(`hook too long (${hook.length})`);
  if (caption.length < 1) reasons.push("caption empty");
  if (caption.length > 280) reasons.push(`caption too long (${caption.length})`);

  for (const re of MOCKING_RES) {
    if (re.test(hook)) reasons.push(`hook mocking ${re}`);
    if (re.test(scene)) reasons.push(`scene mocking ${re}`);
    if (re.test(caption)) reasons.push(`caption mocking ${re}`);
  }
  if (FAMILY_VERB_LEAK_RE.test(scene))
    reasons.push("scene family-verb leak");

  const anchor = pickAnchor(hook, scene);
  if (!anchor) reasons.push("no shared anchor token between hook + scene");

  const hookKey = hook.toLowerCase().trim();
  if (APPROVED_HOOKS.has(hookKey)) reasons.push("dup with approved pack");
  if (DRAFT_HOOKS.has(hookKey)) reasons.push("dup with existing drafts");
  if (seenInBatch.has(hookKey)) reasons.push("dup within batch");
  seenInBatch.add(hookKey);

  const domain = domainFor(title);
  const cluster = clusterFor(title, domain);
  const film = filmFor(domain);

  if (reasons.length > 0) {
    reject.push({ title, hook, scene, anchor, reasons });
    continue;
  }
  ok.push({ title, hook, scene, caption, anchor, domain, cluster, film });
}

const esc = (s) => s.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
const out = [];
out.push(`// ─── BATCH C (BI 2026-05-06) — phone/work/social/content rebalance ──`);
for (const e of ok) {
  out.push("    {");
  out.push(`      hook: "${esc(e.hook)}",`);
  out.push(`      whatToShow:`);
  out.push(`        "${esc(e.scene)}",`);
  out.push(`      howToFilm: ${e.film},`);
  out.push(`      caption: "${esc(e.caption)}",`);
  out.push(`      anchor: "${e.anchor}",`);
  out.push(`      domain: "${e.domain}",`);
  out.push(`      cluster: "${e.cluster}",`);
  out.push(`      pidginLevel: "light_pidgin",`);
  out.push(`      reviewedBy: "BI 2026-05-06",`);
  out.push("    },");
}

console.error(`# parsed=${blocks.length} accepted=${ok.length} rejected=${reject.length}`);
for (const r of reject) {
  console.error(`REJECT [${r.title}] anchor=${r.anchor ?? "—"}: ${r.reasons.join("; ")}`);
  console.error(`  hook: ${r.hook}`);
  console.error(`  scene: ${r.scene.slice(0, 100)}...`);
}
console.log(out.join("\n"));
